...
public class AiChatService {
    ...
    public AiChatMessageResponse chat(AiChatMessageRequest request) {
        ...
        if (hasImages(request)) {
            return new AiChatMessageResponse(request.agentId(), chatWithImages(systemPrompt, request), Instant.now());
        }
    ...
    public Flux<String> stream(AiChatMessageRequest request) {
        ...
        validateChatRequest(request);
        if (hasImages(request)) {
            return Flux.error(new BusinessException(ErrorCode.AI_REQUEST_FAILED));
        }
    ...
    private List<Message> buildConversation(AiChatMessageRequest request) {
        ...
        messages.add(new UserMessage(request.message().trim()));
        return messages;
    }

    private boolean hasImages(AiChatMessageRequest request) {
        return request.images() != null && request.images().stream()
                .anyMatch(image -> image != null && StringUtils.hasText(image.dataUrl()));
    }

    private void validateChatRequest(AiChatMessageRequest request) {
        if (!StringUtils.hasText(request.agentId())) {
            throw new BusinessException(ErrorCode.AI_REQUEST_FAILED);
        }
        if (!StringUtils.hasText(request.message()) && !hasImages(request)) {
            throw new BusinessException(ErrorCode.AI_REQUEST_FAILED);
        }
    }

    private String chatWithImages(String systemPrompt, AiChatMessageRequest request) {
        List<Map<String, Object>> messages = new ArrayList<>();
        messages.add(Map.of("role", "system", "content", systemPrompt));

        if (request.history() != null) {
            for (ChatTurn turn : request.history()) {
                if (turn == null || !StringUtils.hasText(turn.content())) continue;
        ...
        String message = StringUtils.hasText(request.message())
                ...
                : "Please look at the attached image and respond naturally in character.";
        userContent.add(Map.of("type", "text", "text", message));
        request.images().stream()
                .filter(image -> image != null && StringUtils.hasText(image.dataUrl()))
                .forEach(image -> userContent.add(Map.of(
                        "type", "image_url",
                        "image_url", Map.of("url", image.dataUrl(), "detail", "auto")
                )));
        ...
        try {
            ...
            Object choicesRaw = response == null ? null : response.get("choices");
            if (choicesRaw instanceof List<?> choices && !choices.isEmpty()
                    && choices.getFirst() instanceof Map<?, ?> first
                    && first.get("message") instanceof Map<?, ?> messageMap
            ...
            log.warn("Vision chat request failed. model={}, error={}", chatModel, e.getMessage());
            throw new BusinessException(ErrorCode.AI_REQUEST_FAILED);
        }
    }
